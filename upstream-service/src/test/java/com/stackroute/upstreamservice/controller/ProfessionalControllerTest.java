...
public class ProfessionalControllerTest {
    ...
    @Test
    public void sendProfessionalDetailsTest() throws Exception {
//        when(sender.sendMessage(any(), any(), any())).thenReturn(movie);
        mockMvc.perform(MockMvcRequestBuilders.post("/professional")
    ...
    public void sendProfessionalDetailsFailureTest() throws Exception {
        doThrow(new Exception("Professional Details failed to send")).when(sender).sendMessage(any(), any(), any());
        mockMvc.perform(MockMvcRequestBuilders.post("/professional")
                .contentType(MediaType.APPLICATION_JSON).content(asJsonString(data)))
