...
public class PersonalControllerTest {
    ...
    @Test
    public void sendPersonalDetailsTest() throws Exception {
//        when(sender.sendMessage(any(), any(), any())).thenReturn(data);
        mockMvc.perform(MockMvcRequestBuilders.post("/personal")
    ...
    public void sendPersonalDetailsFailureTest() throws Exception {
        doThrow(new Exception("Personal Details failed to send")).when(sender).sendMessage(any(), any(), any());
        mockMvc.perform(MockMvcRequestBuilders.post("/personal")
                .contentType(MediaType.APPLICATION_JSON).content(asJsonString(data)))
